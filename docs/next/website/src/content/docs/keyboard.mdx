...
| Toggle source control sidebar | `prefix+shift+b` |

The source control sidebar docks on the right and shows the repository owning the active workspace's working directory, following it as you switch workspaces, tabs, and panes. While it has focus, Tab moves between the commit message box and the change list; in the list, `j`/`k` or the arrow keys move, `h`/`l` collapse and expand a group, Space stages or unstages the selected file, Enter opens its diff in a popup pane, `a` and `u` stage and unstage everything, `c` jumps to the commit box, `D` twice discards a file's worktree changes (or deletes it, when untracked), `r` refreshes, and Esc closes the panel. Enter in the commit box commits the staged changes, and `ctrl+enter` commits from either side.

Each file row carries inline icons at its right-hand end: `+` stages, `-` unstages, and `↺` discards, matching the keys above. The row of buttons under the branch line is `✓` commit, `↓` pull, `↑` push (or publish, when the branch has no upstream), `⎇` branch, `≡` stash, and `⋯` actions; the last three open dropdowns, which `b`, `S`, and `m` also open from the keyboard. `f` fetches, `p` pulls, and `P` pushes without opening anything. `.` or a right click opens the dropdown for the row under the cursor, which on a commit offers show, cherry-pick, revert, and reset.

In a dropdown, the arrow keys and Enter move and choose, Left or Backspace steps back to the menu you came from, and Esc closes it. Branch lists filter as you type. Destructive rows — discard all, force push, delete branch, drop stash, hard reset — are shown in red and need a second Enter to confirm. The branch dropdown reaches create, checkout, merge, rebase, and delete; the stash dropdown reaches push, pop, apply, and drop; and the actions dropdown collects amend, undo last commit, fetch, pull, push, sync, and the stage/unstage/discard-all commands. While a rebase, merge, or cherry-pick is paused, its continue, skip, and abort rows appear at the top of both the branch and actions dropdowns, and the branch line says which operation is in progress.
